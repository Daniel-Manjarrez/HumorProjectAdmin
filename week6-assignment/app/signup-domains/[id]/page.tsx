...
export default async function SignupDomainDetailPage({ params }: { params: Promise<{ id: string }> }) {
  await requireAdmin();
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-12 px-4 sm:px-6 lg:px-8 transition-colors">
      <div className="max-w-3xl mx-auto relative pt-8">
        ...
        <div className="mb-8">
          <BackButton fallbackUrl="/signup-domains" label="Back to Domains" />
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 shadow-xl rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-700 transition-colors">
          <div className="bg-gray-50 dark:bg-gray-900/50 px-6 py-4 border-b border-gray-200 dark:border-gray-700 transition-colors">
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Signup Domain Details</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400 font-mono mt-1">ID: {domain.id}</p>
          </div>

          <div className="p-6 space-y-6">
            <div>
              <label className="block text-sm font-medium text-gray-500 dark:text-gray-400">Domain</label>
              <p className="mt-1 text-2xl font-bold text-gray-900 dark:text-white font-mono">{domain.apex_domain}</p>
            </div>
