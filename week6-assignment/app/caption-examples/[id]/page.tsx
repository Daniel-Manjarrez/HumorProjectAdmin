      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white dark:bg-gray-800 shadow-xl rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-700 transition-colors">
          <div className="bg-gray-50 dark:bg-gray-900/50 px-6 py-4 border-b border-gray-200 dark:border-gray-700 transition-colors">
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Caption Example Details</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400 font-mono mt-1">ID: {example.id}</p>
          </div>
            ...
            <div className="flex flex-col md:flex-row gap-8">
              ...
              {example.image_id && (
                <div className="w-full md:w-1/3">
                  <div className="bg-gray-100 dark:bg-gray-900 rounded-lg overflow-hidden flex items-center justify-center border border-gray-200 dark:border-gray-700 min-h-[200px] transition-colors">
                    {imageUrl ? (
                  ...
                  <div className="mt-2 text-center">
                    <Link href={`/images/${example.image_id}`} className="text-sm text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300">
                      View Image Details
                    </Link>
                  </div>
                </div>
              ...
              <div className={`w-full ${example.image_id ? 'md:w-2/3' : ''} space-y-6`}>
                <div>
                  <label className="block text-sm font-medium text-gray-500 dark:text-gray-400">Image Description</label>
                  <p className="mt-1 text-gray-800 dark:text-gray-200 bg-gray-50 dark:bg-gray-900/50 p-3 rounded-md transition-colors">{example.image_description}</p>
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-500 dark:text-gray-400">Caption</label>
                  <p className="mt-1 text-xl font-serif text-gray-900 dark:text-white italic bg-gray-50 dark:bg-gray-900/50 p-4 rounded-md border-l-4 border-blue-500 transition-colors">
                    "{example.caption}"
                  </p>
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-500 dark:text-gray-400">Explanation</label>
                  <p className="mt-1 text-gray-800 dark:text-gray-200 bg-gray-50 dark:bg-gray-900/50 p-3 rounded-md transition-colors">{example.explanation}</p>
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-500 dark:text-gray-400">Priority</label>
                    <p className="mt-1 text-lg text-gray-900 dark:text-white">{example.priority}</p>
                  </div>
                  <div>
